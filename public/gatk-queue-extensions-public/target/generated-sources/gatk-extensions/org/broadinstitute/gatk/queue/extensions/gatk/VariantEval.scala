...
class VariantEval extends org.broadinstitute.gatk.queue.extensions.gatk.CommandLineGATK {
...
evalIndexes ++= eval.filter(orig => orig != null).map(orig => new File(orig.getPath + ".idx"))
...
override def commandLine = super.commandLine + optional("-o", out, spaceSeparated=true, escape=true, format="%s") + repeat("-eval", eval, formatPrefix=TaggedFile.formatCommandLineParameter, spaceSeparated=true, escape=true, format="%s") + repeat("-comp", comp, formatPrefix=TaggedFile.formatCommandLineParameter, spaceSeparated=true, escape=true, format="%s") + optional(TaggedFile.formatCommandLineParameter("-D", dbsnp), dbsnp, spaceSeparated=true, escape=true, format="%s") + optional(TaggedFile.formatCommandLineParameter("-gold", goldStandard), goldStandard, spaceSeparated=true, escape=true, format="%s") + conditional(list, "-ls", escape=true, format="%s") + repeat("-select", select_exps, spaceSeparated=true, escape=true, format="%s") + repeat("-selectName", select_names, spaceSeparated=true, escape=true, format="%s") + repeat("-sn", sample, spaceSeparated=true, escape=true, format="%s") + repeat("-knownName", known_names, spaceSeparated=true, escape=true, format="%s") + repeat("-ST", stratificationModule, spaceSeparated=true, escape=true, format="%s") + conditional(doNotUseAllStandardStratifications, "-noST", escape=true, format="%s") + repeat("-EV", evalModule, spaceSeparated=true, escape=true, format="%s") + conditional(doNotUseAllStandardModules, "-noEV", escape=true, format="%s") + optional("-mpq", minPhaseQuality, spaceSeparated=true, escape=true, format=minPhaseQualityFormat) + optional("-mvq", mendelianViolationQualThreshold, spaceSeparated=true, escape=true, format=mendelianViolationQualThresholdFormat) + optional("-ploidy", samplePloidy, spaceSeparated=true, escape=true, format="%s") + optional("-aa", ancestralAlignments, spaceSeparated=true, escape=true, format="%s") + conditional(requireStrictAlleleMatch, "-strict", escape=true, format="%s") + conditional(keepAC0, "-keepAC0", escape=true, format="%s") + optional("-numSamples", numSamples, spaceSeparated=true, escape=true, format="%s") + conditional(mergeEvals, "-mergeEvals", escape=true, format="%s") + optional("-stratIntervals", stratIntervals, spaceSeparated=true, escape=true, format="%s") + optional("-knownCNVs", knownCNVs, spaceSeparated=true, escape=true, format="%s") + conditional(filter_reads_with_N_cigar, "-filterRNC", escape=true, format="%s") + conditional(filter_mismatching_base_and_quals, "-filterMBQ", escape=true, format="%s") + conditional(filter_bases_not_stored, "-filterNoBases", escape=true, format="%s")
}
