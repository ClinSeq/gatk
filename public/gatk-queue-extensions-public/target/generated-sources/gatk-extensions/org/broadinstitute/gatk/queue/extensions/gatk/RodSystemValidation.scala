...
class RodSystemValidation extends org.broadinstitute.gatk.queue.extensions.gatk.CommandLineGATK with ScatterGatherableFunction {
analysisName = "RodSystemValidation"
analysis_type = "RodSystemValidation"
scatterClass = classOf[LocusScatterFunction]

/** Input VCF eval file */
@Input(fullName="eval", shortName="eval", doc="Input VCF eval file", required=true, exclusiveOf="", validation="")
...
/** Should we check that all records at the same site produce equivilent variant contexts */
@Argument(fullName="PerLocusEqual", shortName="", doc="Should we check that all records at the same site produce equivilent variant contexts", required=false, exclusiveOf="", validation="")
var PerLocusEqual: Boolean = _
...
override def commandLine = super.commandLine + repeat("-eval", eval, formatPrefix=TaggedFile.formatCommandLineParameter, spaceSeparated=true, escape=true, format="%s") + optional("-o", out, spaceSeparated=true, escape=true, format="%s") + conditional(PerLocusEqual, "--PerLocusEqual", escape=true, format="%s") + conditional(filter_reads_with_N_cigar, "-filterRNC", escape=true, format="%s") + conditional(filter_mismatching_base_and_quals, "-filterMBQ", escape=true, format="%s") + conditional(filter_bases_not_stored, "-filterNoBases", escape=true, format="%s")
}
