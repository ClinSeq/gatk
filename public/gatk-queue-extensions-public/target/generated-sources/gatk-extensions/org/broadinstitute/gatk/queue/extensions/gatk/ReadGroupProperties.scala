...
class ReadGroupProperties extends org.broadinstitute.gatk.queue.extensions.gatk.CommandLineGATK with ScatterGatherableFunction {
analysisName = "ReadGroupProperties"
analysis_type = "ReadGroupProperties"
...
@Argument(fullName="max_values_for_median", shortName="maxElementsForMedian", doc="Calculate median from the first maxElementsForMedian values observed", required=false, exclusiveOf="", validation="")
var max_values_for_median: Option[Int] = None

/**
 * Short name of max_values_for_median
 * @return Short name of max_values_for_median
 */
def maxElementsForMedian = this.max_values_for_median

/**
 * Short name of max_values_for_median
 * @param value Short name of max_values_for_median
 */
def maxElementsForMedian_=(value: Option[Int]) { this.max_values_for_median = value }
...
override def commandLine = super.commandLine + optional("-o", out, spaceSeparated=true, escape=true, format="%s") + optional("-maxElementsForMedian", max_values_for_median, spaceSeparated=true, escape=true, format="%s") + conditional(filter_reads_with_N_cigar, "-filterRNC", escape=true, format="%s") + conditional(filter_mismatching_base_and_quals, "-filterMBQ", escape=true, format="%s") + conditional(filter_bases_not_stored, "-filterNoBases", escape=true, format="%s")
}
