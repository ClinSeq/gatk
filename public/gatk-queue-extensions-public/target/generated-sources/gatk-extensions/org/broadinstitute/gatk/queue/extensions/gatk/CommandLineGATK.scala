...
class CommandLineGATK extends org.broadinstitute.gatk.queue.function.JavaCommandLineFunction {
...
@Input(fullName="input_file", shortName="I", doc="Input file containing sequence data (SAM or BAM)", required=false, exclusiveOf="", validation="")
...
/** Emit a log entry (level INFO) containing the full list of sequence data files to be included in the analysis (including files inside .bam.list files). */
@Argument(fullName="showFullBamList", shortName="", doc="Emit a log entry (level INFO) containing the full list of sequence data files to be included in the analysis (including files inside .bam.list files).", required=false, exclusiveOf="", validation="")
...
var downsampling_type: org.broadinstitute.gatk.engine.downsampling.DownsampleType = _
 ...
 */
def dt_=(value: org.broadinstitute.gatk.engine.downsampling.DownsampleType) { this.downsampling_type = value }
...
@Argument(fullName="refactor_NDN_cigar_string", shortName="fixNDN", doc="refactor cigar string with NDN elements to one element", required=false, exclusiveOf="", validation="")
...
var validation_strictness: htsjdk.samtools.SAMFileReader.ValidationStringency = _
 ...
 */
def S_=(value: htsjdk.samtools.SAMFileReader.ValidationStringency) { this.validation_strictness = value }
...
var unsafe: org.broadinstitute.gatk.engine.arguments.ValidationExclusion.TYPE = _
 ...
 */
def U_=(value: org.broadinstitute.gatk.engine.arguments.ValidationExclusion.TYPE) { this.unsafe = value }
...
/** Number of given threads to allocate to IO */
@Argument(fullName="num_io_threads", shortName="nit", doc="Number of given threads to allocate to IO", required=false, exclusiveOf="", validation="")
...
@Argument(fullName="num_bam_file_handles", shortName="bfh", doc="Total number of BAM file handles to keep open simultaneously", required=false, exclusiveOf="", validation="")
...
/** Allow interval processing with an unsupported BAM */
@Argument(fullName="allow_intervals_with_unindexed_bam", shortName="", doc="Allow interval processing with an unsupported BAM", required=false, exclusiveOf="", validation="")
