...
class ListAnnotations extends org.broadinstitute.gatk.queue.function.JavaCommandLineFunction {
analysisName = "ListAnnotations"
javaMainClass = "org.broadinstitute.gatk.tools.ListAnnotations"
...
override def commandLine = super.commandLine + optional("-l", logging_level, spaceSeparated=true, escape=true, format="%s") + optional("-log", log_to_file, spaceSeparated=true, escape=true, format="%s") + conditional(help, "-h", escape=true, format="%s") + conditional(version, "-version", escape=true, format="%s")
}
