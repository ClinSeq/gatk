...
class VariantValidationAssessor extends org.broadinstitute.gatk.queue.extensions.gatk.CommandLineGATK with ScatterGatherableFunction {
analysisName = "VariantValidationAssessor"
analysis_type = "VariantValidationAssessor"
...
@Argument(fullName="maxHardy", shortName="", doc="Maximum phred-scaled Hardy-Weinberg violation pvalue to consider an assay valid", required=false, exclusiveOf="", validation="")
var maxHardy: Option[Double] = None

/** Format string for maxHardy */
@Argument(fullName="maxHardyFormat", shortName="", doc="Format string for maxHardy", required=false, exclusiveOf="", validation="")
var maxHardyFormat: String = "%s"

/** Maximum no-call rate (as a fraction) to consider an assay valid */
@Argument(fullName="maxNoCall", shortName="", doc="Maximum no-call rate (as a fraction) to consider an assay valid", required=false, exclusiveOf="", validation="")
var maxNoCall: Option[Double] = None

/** Format string for maxNoCall */
@Argument(fullName="maxNoCallFormat", shortName="", doc="Format string for maxNoCall", required=false, exclusiveOf="", validation="")
var maxNoCallFormat: String = "%s"

/** Maximum homozygous variant rate (as a fraction) to consider an assay valid */
@Argument(fullName="maxHomVar", shortName="", doc="Maximum homozygous variant rate (as a fraction) to consider an assay valid", required=false, exclusiveOf="", validation="")
var maxHomVar: Option[Double] = None

/** Format string for maxHomVar */
@Argument(fullName="maxHomVarFormat", shortName="", doc="Format string for maxHomVar", required=false, exclusiveOf="", validation="")
var maxHomVarFormat: String = "%s"
...
override def commandLine = super.commandLine + required(TaggedFile.formatCommandLineParameter("-V", variant), variant, spaceSeparated=true, escape=true, format="%s") + optional("-o", out, spaceSeparated=true, escape=true, format="%s") + conditional(no_cmdline_in_header, "-no_cmdline_in_header", escape=true, format="%s") + conditional(sites_only, "-sites_only", escape=true, format="%s") + conditional(bcf, "-bcf", escape=true, format="%s") + optional("--maxHardy", maxHardy, spaceSeparated=true, escape=true, format=maxHardyFormat) + optional("--maxNoCall", maxNoCall, spaceSeparated=true, escape=true, format=maxNoCallFormat) + optional("--maxHomVar", maxHomVar, spaceSeparated=true, escape=true, format=maxHomVarFormat) + conditional(filter_reads_with_N_cigar, "-filterRNC", escape=true, format="%s") + conditional(filter_mismatching_base_and_quals, "-filterMBQ", escape=true, format="%s") + conditional(filter_bases_not_stored, "-filterNoBases", escape=true, format="%s")
}
